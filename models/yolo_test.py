    ...
    def forward_once(self, x, x2, profile=False):
        ...
        for m in self.model:
            ...
            if m.f == -4:
                ...
                x = m(x3)
                if x[2].size(1) == 5 or x[2].size(1) == 2: p_dn = x[2]
            else:
                x = m(x)  # run
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args args就是yaml文件里的[256, 20, 20]这个
