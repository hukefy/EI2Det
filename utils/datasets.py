...
from utils.global_var import get_value
...
def create_dataloader_rgb_ir(path1, path2,  imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=LoadImagesAndLabels.collate_fn4 if quad else LoadImagesAndLabels.collate_fn)#数据增强
...
def load_mosaic_RGB_IR(self, index1, index2):
    ...
    for i, index in enumerate(indices):
        ...
        if labels_rgb.size:
            labels_rgb[:, 1:5] = xywhn2xyxy(labels_rgb[:, 1:5], w, h, padw, padh)  # normalized xywh to pixel xyxy format
            labels_ir[:, 1:5] = xywhn2xyxy(labels_ir[:, 1:5], w, h, padw, padh)  # normalized xywh to pixel xyxy format
    ...
    for x in (labels4_rgb[:, 1:5], *segments4_rgb):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    for x in (labels4_ir[:, 1:5], *segments4_ir):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    # img4, labels4 = replicate(img4, labels4)  # replicate
