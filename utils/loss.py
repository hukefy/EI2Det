...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.illumination_loss = nn.CrossEntropyLoss()
    ...
    def __call__(self, p, p_dn, labels):  # predictions, targets, model
        device = labels.device
        targets = labels[:, :6]   # 每行的前六个元素
        n = int(labels[:, 0].max().item())
        k = p_dn.shape[0]
        if n+1 == k:
            dn_labels = torch.full((n+1,), 4, device=device, dtype=torch.long)
            for row in labels:
                index = row[0].item()  # 获取当前行的第一列的值
                dn_labels[int(index)] = row[-1].item()
        else:
            dn_labels = torch.full((k,), 4, device=device, dtype=torch.long)
            for row in labels:
                index = row[0].item()  # 获取当前行的第一列的值
                dn_labels[int(index)] = row[-1].item()
        lcls, lbox, lobj, lrk, i_loss = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors, offsets = self.build_targets(p, targets)  # targets
       
        i_loss = self.illumination_loss(p_dn, dn_labels)
        ...
        i_loss = torch.unsqueeze(i_loss, 0)
        i_loss *= 0.3

        loss = lbox + lobj + lcls + lrk + i_loss
        return loss * bs, torch.cat((lbox, lobj, lcls, lrk, i_loss)).detach()#, i_loss
