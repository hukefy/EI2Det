...
def test(data,
    ...
    for batch_i, (img, nlabels, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            x_out, p_dn = model(img_rgb, img_ir, augment=augment)  # inference and training outputs
            out, _, train_out = x_out
            t0 += time_synchronized() - t

            # Compute loss
            if compute_loss:
                loss += compute_loss([x.float() for x in train_out], p_dn, nlabels)[1][:5]  # box, obj, cls  i_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/MFAD.yaml', help='*.data path')
    ...
    p = "/home/heyudong/RGBT_DETE/datasets/ICA-dataset/labels/test"
